...
# ## Description
#
# This script pushes files from a working directory into a resource in the
# (${XNAT_PBS_JOBS_XNAT_SERVER}) XNAT database.
#
# It does not replace or overwrite the entire specified DB resource. Instead
# it simply places the files from the specified working directory into the
# existing resource. The files will overwrite any existing files in the
...
${g_script_name} - Push files from a working directory into a resource in the
                   XNAT database. Do not replace or overwrite the entire specified
                   resource. Instead simply place the files in the working directory
                   in the resource. The files will overwrite any existing files with
                   the same path.
...
Options: [ ] = optional, < > = user-supplied-value
 ...
 [--use-http]                : If --use-http is specified:
                               ...
                               If --use-http is NOT specified:
                                  ...
                                  It is assume that the specified working directory
                                  (--working-dir=) (containing the files to be pusehd to
                                  the XNAT DB archive) is also available on the XNAT
                                  SERER to which the files are being pushed.
...
get_options()
{
    ...
    if [ -z "${g_resource_suffix}" ]; then
        ...
        log_Msg "resource suffix: ${g_resource_suffix}"
    fi
...
main()
{
    ...
    if [ "${g_use_http}" = "TRUE" ] ; then
        ...
        files=$(fiind ${g_working_dir} -print)
        ...
        for file in ${files} ; do
            ...
            if [ -f ${file} ]; then
                ...
                if [ ! -z "${relative_file_name}" ] ; then
                    log_Msg "Putting file: ${relative_file_name} into DB resource: ${resource}"
        ...
        for file in ${files} ; do
            ...
            if [ -f ${file} ]; then
                ...
                if [ ! -z "${relative_file_name}" ] ; then
                    ...
                    db_file=${file/${g_client_string}/${g_server_string}}
    ...
    log_Msg "Cleanup"
    log_Msg "-------------------------------------------------"
    log_Msg "Removing working dir: ${g_working_dir}"
    rm -rf ${g_working_dir}
}
